...
import { GetELEConfig, MuddleDomain, getSHA224Password, getUUID } from "./helpers"
...
export async function GetELEConfigList(sni: string, addressList: Array<string>, start: number, max: number, env: Env) {
  filterCountries = ""
  proxyList = []
  let configList: Array<Config> = []
  for (let i = 0; i < max; i++) {
    configList.push(GetELEConfig(
      i + start,
      getUUID(sni),
      MuddleDomain(sni),
...
export async function ELEOverWSHandler(request: Request, sni: string, env: Env) {
  ...
  readableWebSocketStream.pipeTo(new WritableStream({
    async write(chunk, controller) {
      ...
      const {
        ...
        addressRemote = "",
        rawClientData,
      } = await ParseELEHeader(chunk, sha224Password);
...
async function ParseELEHeader(buffer: ArrayBuffer, sha224Password: string) {
