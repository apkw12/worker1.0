...
import { GetVlessConfigList } from './AMK'
import { GetTrojanConfigList } from './ELE'
...
export async function GetConfigList(url: URL, env: Env): Promise<Array<Config>> {
  ...
  if (protocols.includes("built-in-AMK")) {
    AMKConfigList = await GetVlessConfigList(url.hostname, cleanDomainIPs, startNo, maxBuiltInConfigsPerType, env)
    startNo += maxBuiltInConfigsPerType
  }
  if (protocols.includes("built-in-ELE")) {
    ELEConfigList = await GetTrojanConfigList(url.hostname, cleanDomainIPs, startNo, maxBuiltInConfigsPerType, env)
    startNo += maxBuiltInConfigsPerType
  }
