...
export function DecodeConfig(configStr: string): Config {
	...
	} else if (configStr.startsWith("AMK://")) {
	  try {
...
export function ValidateConfig(conf: Config): boolean {
	try {
		if (["vmess", "AMK"].includes(conf.configType) && IsValidUUID(conf.uuid as string) && conf.remarks) {
      return !!(conf.address || conf.sni)
		} else if (["ELE"].includes(conf.configType) && (conf.uuid || conf.password) && conf.remarks) {
      return !!(conf.address || conf.sni)
